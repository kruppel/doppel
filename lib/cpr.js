var fs = require('fs')
  ...
  , InvalidSrcError = require('./errors/invalid_src')
  , IdenticalDirsError = require('./errors/identical_dirs')
  , cp = {}
  , cpr = {};
...
cpr.cpr = function (src, dest, callback) {
  if (!src) return callback(new InvalidSrcError());
  ...
  async.waterfall(
    ...
    , function (next) {
        var err = resolvedSrc === resolvedDest ?
          new IdenticalDirsError(resolvedSrc, resolvedDest) :
          undefined;
...
cpr.sync = function (src, dest) {
  if (!src) throw new InvalidSrcError();
